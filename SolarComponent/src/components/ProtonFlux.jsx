...
const ProtonFlux = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const defaultEnergyData = processedData.filter(d => d.energy === '>=10 MeV');
  ...
  const filterByEnergy = (data, energy) => {
    return data.filter(d => d.energy === energy);
  };
  ...
  const updateCharts = () => {
    const energyFilteredData = filterByEnergy(data, selectedEnergy);
    const filteredData = windowed(energyFilteredData, currentPeriod);
    updateChart(mainChartInstance.current, filteredData, { period: currentPeriod });
    
    const miniData = filterByEnergy(data, '>=10 MeV');
    updateChart(miniChartInstance.current, miniData);
  };
  ...
  return (
    <div className="proton-flux-container">
      <h2 className="proton-flux-title">
        Proton Flux | GOES 18 (Pfu)
      </h2>
      
      <div className="proton-flux-status">
        {status}
      </div>
      ...
      <div className="proton-flux-controls">
        ...
        {['6h', '12h', '24h', '3d', '7d', 'all'].map(period => (
          <button
            ...
            className={`proton-flux-period-button ${currentPeriod === period ? 'active' : ''}`}
          >
            {period}
          </button>
        ))}
      ...
      <div className="proton-flux-mini-chart">
        <div className="proton-flux-mini-chart-wrapper">
          <canvas ref={miniChartRef}></canvas>
        </div>
        <div className="proton-flux-mini-chart-label">
          >=10 MeV
        </div>
      </div>
      
      {loading && (
        <div className="proton-flux-loading">
          Loading data...
        </div>
      )}
    </div>
